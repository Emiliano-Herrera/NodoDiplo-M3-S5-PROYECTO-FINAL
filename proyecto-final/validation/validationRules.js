...
export const validarPais = () => [
    ...
    body('capital')
        .isArray({ min: 1 }).withMessage('Las capitales deben ser un array con al menos un elemento')
        .custom((capitales) => {
    ...
    body('capital.*')
        .notEmpty().withMessage('Las capitales no pueden estar vacías')
        .trim()
        .isLength({ min: 3, max: 90 }).withMessage('Cada capital debe tener entre 3 y 90 caracteres'),

    body('habitantes')
        .notEmpty().withMessage('La cantidad de habitantes es requerida')
        .isInt({ min: 1 }).withMessage('La cantidad de habitantes debe ser un número entero positivo')
        .toInt(),

    body('gini')
        .optional({ nullable: true })
        .isFloat({ min: 0, max: 100 }).withMessage('Gini debe ser un número entre 0 y 100')
        .toFloat(),
    ...
    body('lenguajes')
        .isArray({ min: 1 }).withMessage('Debes enviar al menos un lenguaje')
        .custom((lenguajes) => {
    ...
    body('lenguajes.*')
        .trim()
        .isLength({ min: 1 }).withMessage('Cada lenguaje debe tener al menos 1 caracter'),

    body('latitudLongitud')
        .optional()
        .isArray({ min: 2, max: 2 }).withMessage('La latitud y longitud debe ser un array de 2 elementos'),

    body('latitudLongitud.*')
        .isFloat().withMessage('La latitud y longitud deben ser números')
        .toFloat(),

    body('area')
        .notEmpty().withMessage('El area es requerida')
        .isFloat({ min: 0.01 }).withMessage('El area debe ser un número positivo')
        .toFloat(),
